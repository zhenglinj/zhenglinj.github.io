---
typora-root-url: ../
title: "Coursera Machine Learning | 机器学习第7周 支持向量机"
...
![](/images/coursera-machinelearning/3d12b07f13a976e916d0c707fd03153c.png)
...
![](/images/coursera-machinelearning/66facb7fa8eddc3a860e420588c981d5.png)
...
在第一种情况中，假设 $y=1$ ，此时在目标函数中只需有第一项起作用，因为$y=1$时，$(1-y)$项将等于0。因此，当在 $y=1$ 的样本中时，即在 $(x, y)$中 ，我们得到 $-\log(\frac{1}{1+e^{-z}})$这样一项，这里同上一张幻灯片一致。
...
![](/images/coursera-machinelearning/b4b43ee98bff9f5e73d841af1fa316bf.png)
...
![](/images/coursera-machinelearning/ab372c9161375a4f7b6f0bd4a69560e9.png)
...
![](/images/coursera-machinelearning/59541ab1fda4f92d6f1b508c8e29ab1c.png)
...
![](/images/coursera-machinelearning/4ac1ca54cb0f2c465ab81339baaf9186.png)
...
![](/images/coursera-machinelearning/5a63e35db410fdb57c76de97ea888278.png)

最后有别于逻辑回归输出的概率。在这里，我们的代价函数，当最小化代价函数，获得参数${{\theta }}$时，支持向量机所做的是它来直接预测$y$的值等于1，还是等于0。因此，这个假设函数会预测1。当$\theta^Tx$大于或者等于0时，所以学习参数${{\theta }}$就是支持向量机假设函数的形式。那么，这就是支持向量机数学上的定义。
...
![](/images/coursera-machinelearning/cc66af7cbd88183efc07c8ddf09cbc73.png)
...
![](/images/coursera-machinelearning/12ebd5973230e8fdf279ae09e187f437.png)

如果 $C$非常大，则最小化代价函数的时候，我们将会很希望找到一个使第一项为0的最优解。因此，让我们尝试在代价项的第一项为0的情形下理解该优化问题。比如我们可以把$C$设置成了非常大的常数，这将给我们一些关于支持向量机模型的直观感受。

$$\min_\limits{\theta}C\sum_\limits{i=1}^{m}\left[y^{(i)}{\cos}t_{1}\left(\theta^{T}x^{(i)}\right)+\left(1-y^{(i)}\right){\cos}t_{0}\left(\theta^{T}x^{(i)}\right)\right]+\frac{1}{2}\sum_\limits{i=1}^{n}\theta^{2}_{j}$$
...
![](/images/coursera-machinelearning/b1f670fddd9529727aa16a559d49d151.png)

具体而言，如果你考察这样一个数据集，其中有正样本，也有负样本，可以看到这个数据集是线性可分的。我的意思是，存在一条直线把正负样本分开。当然有多条不同的直线，可以把正样本和负样本完全分开。

![](/images/coursera-machinelearning/01105c3afd1315acf0577f8493137dcc.png)
...
![](/images/coursera-machinelearning/e68e6ca3275f433330a7981971eb4f16.png)
...
![](/images/coursera-machinelearning/dd6239efad3d3ee7a89a28574d7795b3.png)
...
![](/images/coursera-machinelearning/f4b6dee99cfb4352b3cac5287002e8de.png)

事实上，支持向量机现在要比这个大间距分类器所体现得更成熟，尤其是当你使用大间距分类器的时候，你的学习算法会受异常点(outlier) 的影响。比如我们加入一个额外的正样本。

![](/images/coursera-machinelearning/b8fbe2f6ac48897cf40497a2d034c691.png)
...
![](/images/coursera-machinelearning/55e05845c636c8c99c03e6e29337d8c4.png)
...
![](/images/coursera-machinelearning/44ad37bce4b7e03835095dccbd2a7b7a.png)
...
![](/images/coursera-machinelearning/03bd4b3ff69e327f7949c3d2a73eed8a.png)

这就是我们先前给出的支持向量机模型中的目标函数。为了讲解方便，我做一点简化，仅仅是为了让目标函数更容易被分析。

![](/images/coursera-machinelearning/3cc61c6e5fe85c2a7bf8170f5bbfd8c3.png)
...
![](/images/coursera-machinelearning/4510b8fbc90ba2b233bb6996529f2df1.png)
...
![](/images/coursera-machinelearning/912cb43058cee46ddf51598b7538968c.png)
...
![](/images/coursera-machinelearning/20725ba601c1c90d1024e50fc24c579b.png)
...
![](/images/coursera-machinelearning/5eab58ad9cb54b3b6fda8f6c96efff24.png)
...
![](/images/coursera-machinelearning/65198a1748fdbe16da34afab9f33d801.png)
...
![](/images/coursera-machinelearning/529b6dbc07c9f39f5266bd0b3f628545.png)
...
...得到$h_\theta (x)=f_1+f_2+...+f_n$。然而，除了对原有的特征进行组合以外，有没有更好的方法来构造$f_1,f_2,f_3$？我们可以利用核函数来计算出新的特征。

给定一个训练样本$x$，我们利用$x$的各个特征与我们预先选定的**地标**(**landmarks**)$l^{(1)},l^{(2)},l^{(3)}$的近似程度来选取新的特征$f_1,f_2,f_3$。

![](/images/coursera-machinelearning/2516821097bda5dfaf0b94e55de851e0.png)
...
![](/images/coursera-machinelearning/b9acfc507a54f5ca13a3d50379972535.jpg)
...
![](/images/coursera-machinelearning/3d8959d0d12fe9914dc827d5a074b564.jpg)
...
![](/images/coursera-machinelearning/eca2571849cc36748c26c68708a7a5bd.png)

![](/images/coursera-machinelearning/ea31af620b0a0132fe494ebb4a362465.png)
